...
void ecrit_chaine_xfig(ListeChaine *L, FILE *f, int zoom, int epaisseur){ 
  ...
  while (chaineCourante != NULL){
	...
	fprintf(f, "         %d %d %d %d\n", (int)(pointPrecedent->x*zoom), (int)(pointPrecedent->y*zoom), (int)(pointCourant->x*zoom), (int)(pointCourant->y*zoom));
      }
      /* afficher le point */
      fprintf(f, "1 4 0 %d 0 0 50 0 20 0.000 1 0.000 %d %d 25 25 0 0 0 0\n", epaisseur, (int)(pointCourant->x*zoom), (int)(pointCourant->y*zoom));
